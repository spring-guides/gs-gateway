...
The following listing shows the Maven build file:
...
The following listing shows the Gradle build file:
...
in circuit breakers. You can do so in the Spring Cloud Gateway by using Hystrix.
...
the host is `hystrix.com`, we route the request to HTTPBin and wrap that request in
a `HystrixCommand`.  We do so by applying a filter to the route. We can configure the Hystrix filter
by using a configuration object. In this example, we give the
`HystrixCommand` a name of `mycmd`.
...
of `hystrix.com`. Otherwise, the request is not routed. We can use the following cURL command:
...
As you can see Hystrix timed out while waiting for the response from HTTPBin. When Hystrix times out,
...
To do so, we can modify our Hystrix filter to provide a URL to call in the case of a timeout:
...
Now, when the Hystrix wrapped route times out, it calls `/fallback` in the Gateway application.
