...
This guide walks you through how to use https://spring.io/projects/spring-cloud-gateway[Spring Cloud Gateway].
...
. Click *Dependencies* and select *Reactive Gateway* and *Resilience4J*.
. Click *Generate*.
...
you can route traffic based on certain conditions as well as alter the request/response as you see fit.
...
Next, add WireMock as a test dependency so you can stand in for HTTPBin during tests.

Maven:
[source,xml]
----
include::complete/pom.xml[tag=wiremock-dependency]
----

Gradle:
[source,groovy]
----
include::complete/build.gradle[tag=wiremock-dependency]
----
...
`@EnableWireMock` starts WireMock before the Spring context loads. `@ConfigureWireMock` maps the mock server's base URL to the `httpbin` property so it matches `UriConfiguration`. 
`@InjectWireMock("httpbin")` gives a handle to register stubs in the test method.

The test sends traffic through the running gateway using `@AutoConfigureWebTestClient` and an injected `WebTestClient`.
